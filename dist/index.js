...
}, withToolLogging("pre_transfer_prep", async () => mkResult({
...
}, withToolLogging("threatmetrix_risk", async () => mkResult({ evaluatedAt: MOCK_NOW_ISO, ...MOCK_RISK_MEDIUM })));
/** 5) 第2暗証取得 */
...
}, withToolLogging("obtain_second_password", async () => mkResult({
    approved: true,
    method: "app",
...
}, withToolLogging("execute_transfer", async ({ fromAccountId, toPayeeId, amountJPY }) => mkResult({
...
}, withToolLogging("update_high_risk_auth_status", async ({ caseId, otpVerified }) => mkResult({
...
}, withToolLogging("term_deposit_prep", async ({ fromAccountId, amountJPY, termMonths, productCode }) => mkResult({
    checkedAt: MOCK_NOW_ISO,
    fromAccountId,
    amountJPY,
...
}, withToolLogging("apply_term_deposit", async ({ fromAccountId, amountJPY, termMonths, productCode, handling }) => mkResult({
